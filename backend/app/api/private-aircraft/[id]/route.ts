...
// ========== GET /api/private-aircraft/[id] ==========
async function getHandler(req: AuthenticatedRequest, { params }: Ctx) {
  try {
    const user = req.user!;
...
// ========== PUT /api/private-aircraft/[id] ==========
async function putHandler(req: AuthenticatedRequest, { params }: Ctx) {
  try {
    const user = req.user!;
    const id = parseInt(params.id, 10);
...
// ========== DELETE /api/private-aircraft/[id] ==========
async function deleteHandler(req: AuthenticatedRequest, { params }: Ctx) {
  try {
    const user = req.user!;
    const id = parseInt(params.id, 10);
