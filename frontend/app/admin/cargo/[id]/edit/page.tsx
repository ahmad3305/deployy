...
const STATUS_ENUM = [
  "Booked", "Loaded", "In Transit", "Unloaded", "Customs Hold", "Delivered", "Cancelled"
];
...
export default function EditCargoPage() {
  ...
  useEffect(() => {
    async function loadCargo() {
      ...
      try {
        const token = localStorage.getItem("token") || "";
        const res = await fetch(`${API_BASE}/cargo/${id}`, {
        ...
        setForm({
          ...json.data,
          is_insured: !!json.data.is_insured,
        });
      } catch (e: any) {
  ...
  return (
    <div style={styles.bg}>
      <div style={styles.shell}>
        <h2 style={styles.title}>Edit Cargo Record</h2>
        <div style={styles.card}>
          ...
          {loading ? (
            <div style={styles.loading}>Loading cargo info...</div>
          ) : (
