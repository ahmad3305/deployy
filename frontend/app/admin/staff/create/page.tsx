...
// Match database ENUM constraints exactly
const STAFF_ROLES = [
  "Pilot",
  "Co-Pilot",
  "Cabin Crew",
...
const STAFF_TYPES = ["Flight Crew", "Ground Staff"];
...
export default function StaffCreatePage() {
  ...
  useEffect(() => {
    ...
    console.log("Checking admin auth for create staff, user:", user);
  ...
  // Load airports only if authorized
  useEffect(() => {
    ...
    async function loadAirports() {
      ...
      try {
        ...
        console.log("Airports response:", json);

        if (!res.ok)
          throw new Error(json.message || "Failed to load airports");
        setAirports(json.data as Airport[]);
      } catch (e: any) {
        console.error("Fetch airports error:", e);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.log("Creating staff with token:", token.substring(0, 20) + "...");
      ...
      console.log("Create staff response status:", res.status);

      const json = await res.json();
      console.log("Create staff response:", json);

      if (!res.ok) {
        // Handle validation errors - convert to string array
        ...
        <div style={styles.card}>
          ...
          {apiErrors.length > 0 && (
            <div style={styles.errorList}>
              {apiErrors.map((err, n) => (
                <div key={n} style={{ marginBottom: n < apiErrors.length - 1 ? 8 : 0 }}>
                  • {err}
                </div>
              ))}
            </div>
